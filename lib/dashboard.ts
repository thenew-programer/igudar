...
export class DashboardService {
  ...
  static async getPortfolioBreakdown(userId: string): Promise<DatabaseResponse<PortfolioBreakdown[]>> {
    try {
      ...
      const totalValue = Object.values(breakdown).reduce((sum, item) => sum + item.value, 0);

      const portfolioBreakdown: PortfolioBreakdown[] = Object.entries(breakdown).map(([type, data], index) => {
        const colors = [
          'bg-igudar-primary',
