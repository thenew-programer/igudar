...
export const StatsCards: React.FC = () => {
  ...
  const fetchStats = async () => {
      ...
      try {
        ...
        const result = await DashboardService.getUserStats(user.id);
  ...
  const generateStatCards = (data: DashboardStats): StatCard[] => {
    return [
      {
        title: 'Total Invested',
        value: formatCurrency(data.totalInvested),
        change: data.totalInvested > 0 ? '+12.5%' : '0%',
        changeType: 'positive',
        icon: DollarSign,
      ...
      },
      {
        ...
        change: data.activeInvestments > 0 ? `+${data.activeInvestments}` : '0',
        changeType: 'positive',
        icon: Building2,
      ...
      },
      {
        ...
        change: formatPercentage(data.roiPercentage),
        changeType: data.roiPercentage >= 0 ? 'positive' : 'negative',
        icon: TrendingUp,
